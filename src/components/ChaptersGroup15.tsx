...
function G3({ items }: { items: { icon: string; judul: string; teks: string; warna: string; bg: string }[] }) {
  return (
...
function Ch151() {
  ...
  return (
    <CP id="ch151" num="151" title="KEPEMIMPINAN TECH — DARI DEVELOPER KE LEADER" color="#1a5cff" bg="#e8f0ff">
...
// ── Ch152: GENERASI BANGSA ────────────────────────────────────────────────────
function Ch152() {
  return (
    <CP id="ch152" num="152" title="GENERASI BANGSA — TONGKAT ESTAFET DIGITAL" color="#e63329" bg="#fef2f2">
      ...
      <G3 items={[
        ...
        { icon: '🌾', judul: 'Desa Digital', teks: 'Bawa teknologi ke desa bukan cuma sinyal — tapi skill dan mindset digital untuk warga desa', warna: '#22c55e', bg: '#f0fdf4' },
...
// ── Ch153: FILOSOFI HIDUP DEVELOPER ──────────────────────────────────────────
function Ch153() {
  const filsafat = [
    { filsuf: 'Stoicism', relevan: 'Kendalikan yang bisa dikontrol (kode sendiri), terima yang tidak (requirement berubah di menit terakhir)', icon: '🏛️', warna: '#8b5cf6' },
  ...
  return (
    <CP id="ch153" num="153" title="FILOSOFI DEVELOPER — HIKMAH DALAM KODE" color="#8b5cf6" bg="#f5f0ff">
      <div className="speech-bubble inline-block text-sm mb-6">🏛️ Developer terbaik bukan yang paling pintar — tapi yang punya prinsip hidup yang benar!</div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filsafat.map((f, i) => (
...
// ── Ch154: DIVERSITY & INCLUSION IN TECH ─────────────────────────────────────
function Ch154() {
  return (
    <CP id="ch154" num="154" title="DIVERSITY — TEKNOLOGI UNTUK SEMUA MANUSIA" color="#e1306c" bg="#fff0f5">
      <div className="speech-bubble-right inline-block text-sm mb-6 text-[#0a0a0a]">
...
// ── Ch155: DIGITAL CITIZENSHIP ────────────────────────────────────────────────
function Ch155() {
  return (
    <CP id="ch155" num="155" title="DIGITAL CITIZENSHIP — WARGA DIGITAL YANG BAIK" color="#1a5cff" bg="#e8f0ff">
      <div className="speech-bubble inline-block text-sm mb-6">🌐 Di dunia digital, setiap klik adalah tindakan — jadilah warga digital yang bertanggung jawab!</div>
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-3">
          {[
            ...
            { aspek: 'Screen Time', desc: 'Notifikasi, doomscrolling, dan filter bubble adalah fitur yang sengaja dirancang. Sadar dan batasi!', icon: '⏱️', warna: '#f59e0b' },
            { aspek: 'Online Kindness', desc: 'Di balik setiap akun ada manusia nyata. Treat others online as you would offline. No cyberbullying!', icon: '💚', warna: '#e1306c' },
          ].map((a, i) => (
...
function Ch156() {
  return (
    <CP id="ch156" num="156" title="INOVASI LOKAL — SOLUSI UNTUK MASALAH INDONESIA" color="#e63329" bg="#fef2f2">
      <div className="speech-bubble-right inline-block text-sm mb-6 text-[#0a0a0a]">
      ...
      <G3 items={[
        ...
        { icon: '🎓', judul: 'Bahasa Daerah', teks: 'NLP untuk 300+ bahasa daerah Indonesia — Google Translate belum cover semua. Ini research opportunity!', warna: '#8b5cf6', bg: '#f5f0ff' },
        ...
        { icon: '🌊', judul: 'Bencana Alam', teks: 'Sistem peringatan dini bencana berbasis IoT dan SMS (karena internet tidak selalu ada saat bencana)', warna: '#f59e0b', bg: '#fffbeb' },
      ]} />
    </CP>
  )
}

// ── Ch157: GENERASI ALPHA & AI ────────────────────────────────────────────────
function Ch157() {
  return (
    <CP id="ch157" num="157" title="GENERASI ALPHA — ANAK-ANAK DI ERA AI" color="#f59e0b" bg="#fffbeb">
      ...
      <G3 items={[
        ...
        { icon: '📱', judul: 'Screen Concern', teks: 'Screen time di usia dini berpengaruh ke perkembangan otak. Balance antara digital dan offline sangat penting', warna: '#e63329', bg: '#fef2f2' },
...
// ── Ch158: TEKNOLOGI & AGAMA ──────────────────────────────────────────────────
function Ch158() {
  return (
    <CP id="ch158" num="158" title="TEKNOLOGI & SPIRITUALITAS — HARMONI HIDUP" color="#22c55e" bg="#f0fdf4">
      <div className="speech-bubble-right inline-block text-sm mb-6 text-[#0a0a0a]">
...
// ── Ch159: KEARIFAN LOKAL & DIGITAL ──────────────────────────────────────────
function Ch159() {
  return (
    <CP id="ch159" num="159" title="KEARIFAN LOKAL — DIGITALISASI BUDAYA" color="#8b5cf6" bg="#f5f0ff">
      ...
      <G3 items={[
        ...
        { icon: '📜', judul: 'Aksara Daerah', teks: 'Font digital aksara Jawa, Sunda, Bugis — keyboard input dan OCR untuk lestarikan tulisan tradisional', warna: '#22c55e', bg: '#f0fdf4' },
...
function Ch160() {
  const timeline = [
    ...
    { tahun: '1991', event: 'World Wide Web oleh Tim Berners-Lee. Internet jadi bisa diakses publik!', icon: '📡', warna: '#8b5cf6' },
    ...
    { tahun: '2026', event: 'Rizki Habibi membangun KVT.kom — platform edukasi developer dari Jember untuk Indonesia! 🎉', icon: '🚀', warna: '#ffd700' },
  ]
  return (
    <CP id="ch160" num="160" title="SEJARAH COMPUTING — BAHU DI MANA KITA BERDIRI" color="#1a5cff" bg="#e8f0ff">
      <div className="speech-bubble-right inline-block text-sm mb-6 text-[#0a0a0a]">
