...
function Ch71() {
  ...
  return (
    <CP id="ch71" chNum="CHAPTER 71" title="DEVELOPER SEHAT — BODY & MIND" color="#22c55e" bg="#f0fdf4">
...
function Ch72() {
  ...
  return (
    <CP id="ch72" chNum="CHAPTER 72" title="CODING WITH MUSIC — SOUNDTRACK HIDUP" color="#1a5cff" bg="#e8f0ff">
          ...
          <div className="space-y-3">
            {genre.map((g, i) => (
              <motion.div key={g.nama}
                ...
                viewport={{ once: false }}
                className="p-3"
                style={{ border: `2px solid ${g.warna}`, boxShadow: `3px 3px 0 ${g.warna}`, background: 'white' }}>
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-xl">{g.emoji}</span>
                  <span className="font-comic text-sm text-[#0a0a0a]">{g.nama}</span>
                  <span className="ml-auto font-comic text-sm" style={{ color: g.warna }}>{g.power}%</span>
                </div>
                <div className="h-2.5" style={{ border: '1.5px solid #0a0a0a', background: '#f0f0eb' }}>
                  <motion.div
                    initial={{ width: 0 }}
                    whileInView={{ width: `${g.power}%` }}
                    transition={{ duration: 1.2, delay: i * 0.1 }}
                    viewport={{ once: false }}
                    style={{
                      height: '100%',
                      background: `repeating-linear-gradient(-45deg,${g.warna} 0,${g.warna} 5px,${g.warna}88 5px,${g.warna}88 10px)`,
                    }}
                  />
                </div>
                <p className="text-[10px] text-[#0a0a0a]/60 font-bold mt-1">{g.deskripsi}</p>
              </motion.div>
            ))}
          </div>
        </div>
...
function Ch73() {
  ...
  return (
    <CP id="ch73" chNum="CHAPTER 73" title="PERPUSTAKAAN DIGITAL — SUMBER ILMU" color="#f59e0b" bg="#fffbeb">
      <div className="speech-bubble inline-block text-sm mb-6">
        📚 Belajar tidak pernah berhenti — setiap hari adalah chapter baru!
      </div>
      <div className="grid sm:grid-cols-2 gap-4">
        {bacaan.map((b, i) => (
          <motion.div key={b.kategori}
            initial={{ opacity: 0, scale: 0.9, rotate: i % 2 === 0 ? -1 : 1 }}
            whileInView={{ opacity: 1, scale: 1, rotate: 0 }}
            transition={{ delay: i * 0.1, type: 'spring' }}
            viewport={{ once: false }}
            whileHover={{ y: -4 }}
            style={{ border: `3px solid ${b.warna}`, boxShadow: `5px 5px 0 ${b.warna}`, background: 'white', overflow: 'hidden' }}>
            <div className="flex items-center gap-2 px-4 py-2" style={{ background: b.warna, borderBottom: '2px solid #0a0a0a' }}>
              <span className="text-xl">{b.icon}</span>
              <span className="font-comic text-sm text-white">{b.kategori}</span>
            </div>
            <div className="p-4 space-y-2">
              {b.items.map(item => (
                <div key={item} className="flex items-center gap-2 text-xs font-bold text-[#0a0a0a]">
                  <div className="w-1.5 h-1.5 flex-shrink-0" style={{ background: b.warna }} />
                  {item}
                </div>
              ))}
            </div>
...
function Ch74() {
  ...
  return (
    <CP id="ch74" chNum="CHAPTER 74" title="KREATIVITAS — SISI LAIN SANG DEVELOPER" color="#8b5cf6" bg="#f5f0ff">
...
function Ch75() {
  ...
  return (
    <CP id="ch75" chNum="CHAPTER 75" title="KOMUNITAS — KEKUATAN BERSAMA" color="#22c55e" bg="#f0fdf4">
      <div className="grid lg:grid-cols-2 gap-8">
        <div>
...
function Ch76() {
  ...
  return (
    <CP id="ch76" chNum="CHAPTER 76" title="IMPIAN BESAR — BEYOND THE HORIZON" color="#ffd700" bg="#fffbeb" dark>
...
function Ch77() {
  ...
  return (
    <CP id="ch77" chNum="CHAPTER 77" title="SURAT UNTUK DEVELOPER MUDA" color="#1a5cff" bg="#e8f0ff">
      <div className="speech-bubble-right inline-block text-sm mb-6 text-[#0a0a0a]">
...
function Ch78() {
  ...
  return (
    <CP id="ch78" chNum="CHAPTER 78" title="IOT INOVASI — DARI BARANG BEKAS JADI KARYA" color="#22c55e" bg="#f0fdf4">
      <div className="speech-bubble inline-block text-sm mb-6">
...
function Ch79() {
  ...
  return (
    <CP id="ch79" chNum="CHAPTER 79" title="MASA DEPAN WEB — TREN TEKNOLOGI" color="#8b5cf6" bg="#f5f0ff">
      <div className="speech-bubble-right inline-block text-sm mb-6 text-[#0a0a0a]">
...
function Ch80() {
  return (
    <CP id="ch80" chNum="CHAPTER 80" title="TERIMA KASIH TELAH MEMBACA!" color="#ffd700" bg="#fffbeb" dark>
