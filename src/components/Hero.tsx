...
import { FiArrowDown, FiAward, FiCode, FiZap, FiStar, FiGlobe } from 'react-icons/fi'

const typingTexts = ['Web Developer', 'Laravel Engineer', 'Next.js Creator', 'UI/UX Explorer', 'BNSP Certified', 'IoT Innovator']
...
function TypingText() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (!del) { if (ci < cur.length) setCi(c => c + 1); else setTimeout(() => setDel(true), 1500) }
      else { if (ci > 0) setCi(c => c - 1); else { setDel(false); setTi(i => (i + 1) % typingTexts.length) } }
    }, del ? 40 : 80)
    return () => clearTimeout(t)
  }, [ci, del, ti])
  return <span className="text-comic-blue">{typingTexts[ti].slice(0, ci)}<span className="typing-cursor">|</span></span>
}

const stars = [
  { top: '8%', left: '4%', size: 28, delay: 0 }, { top: '22%', left: '91%', size: 22, delay: 0.3 },
  { top: '60%', left: '2%', size: 20, delay: 0.6 }, { top: '78%', left: '87%', size: 24, delay: 0.9 },
  { top: '42%', left: '95%', size: 16, delay: 1.2 }, { top: '88%', left: '40%', size: 18, delay: 0.5 },
  { top: '15%', left: '55%', size: 14, delay: 0.8 }, { top: '50%', left: '48%', size: 12, delay: 1.4 },
]
...
function Star({ top, left, size, delay }: { top: string, left: string, size: number, delay: number }) {
  return (
    <motion.div style={{ position: 'absolute', top, left, width: size, height: size, pointerEvents: 'none', zIndex: 1 }}
      animate={{ scale: [1, 1.5, 1], rotate: [0, 20, -20, 0] }} transition={{ duration: 2.5, repeat: Infinity, delay }}>
...
const comicEffects = ['ZAP!', 'POW!', 'WOW!', 'NICE!', 'COOL!', 'EPIC!']

// Mini stat panels untuk landing
const miniStats = [
...
// Tech chips landing
const techChips = ['Laravel', 'Next.js', 'PHP', 'MySQL', 'Tailwind', 'IoT', 'AI/ML']

// Achievement unlocks landing
...
export default function Hero() {
  ...
  const handleToggle = () => {
    const fx = comicEffects[Math.floor(Math.random() * comicEffects.length)]
    setShowEffect(fx); setEffectKey(k => k + 1); setIsKomik(v => !v)
    setTimeout(() => setShowEffect(''), 900)
  }
          ...
          <div className="lg:col-span-3 order-2 lg:order-1 space-y-3 sm:space-y-4 w-full min-w-0">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65 }} className="flex flex-wrap gap-2 sm:gap-3 mt-1">
              {[
                ...
                { icon: '📅', num: '3+', label: 'TAHUN' },
                { icon: '🏆', num: '75+', label: 'SERTIFIKAT' },
