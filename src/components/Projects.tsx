...
export default function Projects() {
  ...
  const proyekTersaring = aktifKategori === 'SEMUA'
    ? projects
    : projects.filter(p => p.kategori === aktifKategori)
  ...
  return (
    <section id="projects" className="py-16 sm:py-20 px-3 sm:px-4 relative" style={{ background: '#fafaf7' }}>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {/* Chapter Header */}
        <ChapterHeader nomor="03" judul="THE MISSIONS" warna="#1a5cff" subtitle="🎯 Setiap proyek adalah misi yang harus diselesaikan!" />
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {proyekTersaring.map((project, index) => (
              <motion.div
                key={project.id}
              ...
              >
                ...
                >
                  <span className="font-comic text-white text-base sm:text-xl tracking-widest">PROJECT #{project.num}</span>
                ...
                {/* Image */}
                <div className="relative h-36 sm:h-44 overflow-hidden" style={{ background: project.bgColor }}>
                  {project.image ? (
                    <Image
                      src={project.image}
                      alt={project.title}
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                      ...
                      <FiFolder className="w-12 h-12 sm:w-14 sm:h-14 relative z-10" style={{ color: project.color }} />
                      <span className="font-comic text-xs sm:text-sm relative z-10" style={{ color: project.color }}>
                        {project.tech[0]}
                      </span>
                    </div>
                  )}
                ...
                <div className="p-3 sm:p-4 bg-white">
                  ...
                  <p className="text-[10px] sm:text-xs font-bold text-comic-black/50 mb-2 uppercase tracking-wide">{project.tagline}</p>
                  <p className="text-xs sm:text-sm text-comic-black/70 mb-3 sm:mb-4 leading-relaxed line-clamp-2">{project.description}</p>
        ...
        {selected && (
          ...
          >
            ...
            >
              ...
              >
                <span className="font-comic text-white text-base sm:text-xl">PROJECT #{selected.num} — CASE FILE</span>
                <button
                  onClick={() => setSelected(null)}
              ...
              {/* Modal image */}
              <div className="relative h-40 sm:h-52 border-b-3 border-comic-black" style={{ borderBottom: '3px solid #0a0a0a', background: selected.bgColor }}>
                {selected.image ? (
                  <Image src={selected.image} alt={selected.title} fill className="object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <FiFolder className="w-16 h-16 sm:w-20 sm:h-20" style={{ color: selected.color }} />
                  </div>
                )}
              </div>
              ...
              <div className="p-4 sm:p-6 space-y-4">
                <h2 className="font-comic text-2xl sm:text-3xl text-comic-black">{selected.title}</h2>
                <p className="text-xs sm:text-sm text-comic-black/70 leading-relaxed">{selected.description}</p>
                ...
                {[
                  ...
                  <div key={item.label} className="p-3 sm:p-4 border-2" style={{ background: item.bg, borderColor: item.border, boxShadow: `3px 3px 0 ${item.border}` }}>
                    <div className="font-comic text-xs sm:text-sm mb-1" style={{ color: item.border }}>{item.label}</div>
                    <p className="text-xs sm:text-sm text-comic-black">{item.text}</p>
                  </div>
                ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.tech.map((t) => (
                      <span key={t} className="font-bold text-xs px-2 sm:px-3 py-1 border-2"
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  <a href={selected.demo} className="btn-comic text-xs sm:text-sm py-2 px-4 sm:px-5">
                    OPEN PROJECT →
                  </a>
                  <a href={selected.github} className="btn-comic-outline text-xs sm:text-sm py-2 px-4 sm:px-5 flex items-center gap-2">
