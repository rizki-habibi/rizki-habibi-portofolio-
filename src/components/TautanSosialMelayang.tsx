...
import { FiGithub, FiMail, FiTarget, FiLink } from 'react-icons/fi'
...
const items = [
  ...
  // Ikon bintang/target untuk tujuan karir — ganti dari ungu kerjasama
  { Icon: FiTarget, href: '#tujuan-karir', label: 'Tujuan', warna: '#22c55e', ext: false },
]
...
// ── Mobile bottom bar — icon + label kecil ─────────────────────────────
function BottomBarMobile() {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden"
      style={{ background: '#0a0a0a', borderTop: '2px solid #1a5cff22' }}>
      <div className="flex items-center justify-around px-1 py-1.5">
        {items.map(({ Icon, href, label, warna, ext }) => (
          <a
            ...
            title={label}
            className="flex flex-col items-center gap-0.5 px-1.5 py-1 min-w-0"
            style={{ textDecoration: 'none' }}
          >
            <div className="flex items-center justify-center rounded"
              style={{ width: 32, height: 32, background: warna + '20', border: `1.5px solid ${warna}55` }}>
              <Icon className="w-3.5 h-3.5" style={{ color: warna }} />
            </div>
            <span className="font-bold text-[8px] truncate max-w-[40px]"
              style={{ color: warna + 'aa' }}>
              {label}
            </span>
          </a>
        ))}
